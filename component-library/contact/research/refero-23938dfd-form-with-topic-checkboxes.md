...
A form-led contact pattern where the LEFT column previews "what you'll get out of submitting" via a small topic-checkbox list ("Discuss pricing", "Assess your team's needs", "Learn more about features") with a friendly portrait photo, and the RIGHT column is the form itself. Below the two-column band, a flat regional phone-number grid catches users who want a number instead of a form. The checkbox preview reframes the form from "give us your info" to "here's what we'll talk about" — a conversion move that's archetype-neutral.

## What to take
- Two-column band: left column is heading + 1-paragraph supporting copy + 3 small topic-checkbox rows + a portrait photo with a soft abstract shape behind it. Right column is the form (Name → Email → Phone → 1–2 dropdowns → submit).
- The topic-checkboxes use a small filled-square check (not a checkbox the user toggles) — they're informative, not interactive. Their job is to set expectations for what the conversation will cover.
- Form fields use light-grey input backgrounds with no visible border, just a subtle inset. Labels sit ABOVE dropdowns and the field label is a real question ("What department do you work in?") not a noun ("Department").
- The portrait photo is a single person looking down at a laptop, lit warmly, with a soft watercolor-style shape behind — gives the form section a human anchor without drifting into stock-imagery territory.
- Below the two-column band, a wide light-band section with regional phone numbers grouped in 4 region columns: North America / Europe / Latin America / Pacific. Each region heading bold, numbers as flat rows. No icons, no cards.
...
- FIELD: portrait photo of a real farmer or equipment operator (not a smiling stock professional). Replace watercolor shape with a torn-edge cream paper texture. Submit button in warm amber.
...
Sibling note: see the [two-card decision pivot](./refero-376ca585-two-card-decision-pivot.md) for the alternative "intent-route first, form later" pattern. Related: [CLEAN archetype](../../../archetypes/clean.md), [content/service-patterns.md](../../../content/service-patterns.md) for the canonical Sam Welton contact pattern.
