...
- [component-library/product-block/research/refero-8f7c3944-guided-product-finder.md](component-library/product-block/research/refero-8f7c3944-guided-product-finder.md) — multi-step guided product finder wizard: step 1 photo-tile use-case selector ("I am looking for...") → steps 2–5 plain radio refinements → persistent Back/Continue + thin progress bar → single-product recommendation card with "Why this is right for you" rationale; for dealers whose customers don't yet know whether they want R-32 vs I-19 vs an accessory bundle (all 6 archetypes via per-archetype palette)
...
- [component-library/services/research/refero-d030e2b2-numbered-step-process.md](component-library/services/research/refero-d030e2b2-numbered-step-process.md) — process-led services pattern: two-column hero with photo card + 3-step horizontal numbered timeline (connecting rule between circles) + ZIP/region locator + "How it works" 3-up cards on dark band + alternating cream/dark testimonial-FAQ-CTA bands; right pick when the customer's hesitation is "what does this even involve?" Two registers in one note: process-walkthrough AND enrollment / become-a-member (with optional intensifiers — multi-quote saturated-brand testimonial band, default-expand-highest-friction FAQ, closer signup band that mirrors the hero) (all 6 archetypes via per-archetype palette)
...
- [component-library/contact/research/refero-1835be20-single-location-showroom.md](component-library/contact/research/refero-1835be20-single-location-showroom.md) — single-location showroom-as-venue contact: store-name page title + two-column header (hours/address left, embedded map right) + "What's Happening In Store" 3-up event cards + flat list of bookable services-and-appointments rows; for single-location dealers whose physical place IS the credibility move (also the per-store template that the dealer-locator-map routes pin-clicks to for multi-location networks) (all 6 archetypes via per-archetype palette)
