...
- [component-library/product-block/research/refero-96bb6cc1-feature-card-mosaic.md](component-library/product-block/research/refero-96bb6cc1-feature-card-mosaic.md) — feature-card mosaic with mixed dark/light tones: contained rounded-corner cards in an asymmetric grid (1 wide hero card → 2-up rows → 1 wide closer), each card title + paragraph + photograph; dark/cream alternation per cell carries the rhythm + "In the Box" component strip + accessory cross-sell rail with quantity steppers + product FAQ accordion (all 6 archetypes via per-archetype palette)
...
- [component-library/services/research/refero-d030e2b2-numbered-step-process.md](component-library/services/research/refero-d030e2b2-numbered-step-process.md) — process-led services pattern (canonical, merged 2026-05-07): two-column hero with photo card + 3-step horizontal numbered timeline (connecting rule between circles) + ZIP/region locator + "How it works" 3-up cards on dark band + alternating cream/dark testimonial-FAQ-CTA bands; covers both service-application explainer and enrollment-led recruitment-program variants (Part 107 cohort, fleet-share, certification ladder) with multi-quote testimonial band on saturated brand-color + smart-default FAQ (all archetypes via per-archetype palette)
- [component-library/services/research/refero-a52e1484-protection-plan-explainer.md](component-library/services/research/refero-a52e1484-protection-plan-explainer.md) — single-plan protection / extended-warranty explainer: SKU-style buy-band + price + disclaimer at top → saturated brand-color billboard with plan wordmark → 5–7 stacked named "promises" each with real photograph + concrete coverage detail; transactional (cart) not lead-gen (form); right pick for paid extended-service plans, Part 137 maintenance subscriptions, fleet protection programs (all 6 archetypes via per-archetype palette)
...
- [component-library/contact/research/refero-3acf3200-client-services-strip-appointment.md](component-library/contact/research/refero-3acf3200-client-services-strip-appointment.md) — client-services secondary-nav strip + ambient-place photograph + simplified appointment-request form (no live calendar grid, asynchronous human follow-up); right pick when the dealer has a real physical place + 5–8 in-person service categories (visits, demo flights, service drop-off, training enrollment) (all 6 archetypes via per-archetype palette + ambient-store photograph)
