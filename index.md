...
- [component-library/services/research/refero-d030e2b2-numbered-step-process.md](component-library/services/research/refero-d030e2b2-numbered-step-process.md) — process-led services pattern: two-column hero with photo card + 3-step horizontal numbered timeline (connecting rule between circles) + ZIP/region locator + "How it works" 3-up cards on dark band + alternating cream/dark testimonial-FAQ-CTA bands; merges three captures of the same screen and now covers two registers — (1) service-explanation when the customer's hesitation is "what does this even involve?" and (2) enrollment-program variant (saturated brand-color hero/closer bookend + multi-quote testimonial band + smart-default FAQ + benefit-cards-with-attribution) for Part 107 cohort / fleet-share / operator-certification recruitment (all archetypes via per-archetype palette)
...
- [component-library/contact/research/refero-1835be20-single-storefront-store-page.md](component-library/contact/research/refero-1835be20-single-storefront-store-page.md) — single-storefront "store-as-page" pattern: store-header → 2-column store-details + embedded map → "What's Happening In Store" 3-up local-events row with RSVP CTAs → "Services & Appointments" vertical stack with per-row Book Now CTAs; place-anchored counterpart to the multi-location dealer-locator-map (all archetypes via per-archetype palette; right pick when the dealer has a real physical showroom with bookable services and real in-person events)
